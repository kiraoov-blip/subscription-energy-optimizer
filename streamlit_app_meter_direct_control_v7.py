...
APP_VERSION = "2026-07-14-direct-control-v7.0"
HOURS = np.arange(24)
DAYS_IN_MONTH = 30
...
DIRECT_CONTROL_MODES = {
    "편의 우선": {"package_style": "최소불편형", "shift_fraction": 0.30, "delivery_factor": 0.94},
    "균형": {"package_style": "균형형", "shift_fraction": 0.55, "delivery_factor": 0.97},
    "목표달성 우선": {"package_style": "목표달성 우선형", "shift_fraction": 0.75, "delivery_factor": 0.99},
}

DIRECT_ACTION_META = {
    "standby": {"device": "대기전력·스마트플러그", "component": "fixed", "max_fraction": 0.10, "hours": list(range(0, 8)) + list(range(9, 17)), "method": "취침·외출 자동 차단"},
    "hvac_setpoint": {"device": "냉난방기", "component": "hvac", "max_fraction": 0.16, "hours": list(range(24)), "method": "설정온도·외출모드 자동조정"},
    "hvac_hour": {"device": "냉난방기", "component": "hvac", "max_fraction": 0.22, "hours": list(range(24)), "method": "예냉·예열 후 운전시간 단축"},
    "dryer_skip": {"device": "건조기", "component": "shift", "max_fraction": 0.55, "hours": list(range(9, 24)), "method": "승인된 횟수만 자연건조 전환"},
    "dishwasher_eco": {"device": "식기세척기", "component": "shift", "max_fraction": 0.18, "hours": list(range(18, 24)) + list(range(0, 7)), "method": "절전모드·모아서 운전"},
    "laundry_eco": {"device": "세탁기", "component": "shift", "max_fraction": 0.12, "hours": list(range(6, 24)), "method": "냉수·절전코스 자동선택"},
    "game_tv": {"device": "게임콘솔·TV", "component": "behavior", "max_fraction": 0.50, "hours": list(range(18, 24)), "method": "고객 설정시간 이후 종료·절전"},
    "aircare": {"device": "공기청정기·제습기", "component": "fixed", "max_fraction": 0.10, "hours": list(range(24)), "method": "자동·저속모드와 예약종료"},
    "clothing_care": {"device": "의류관리기", "component": "shift", "max_fraction": 0.28, "hours": list(range(17, 24)) + list(range(0, 8)), "method": "승인된 운전횟수 축소"},
    "robot_vacuum": {"device": "로봇청소기", "component": "shift", "max_fraction": 0.12, "hours": list(range(8, 18)), "method": "오염도 낮은 날 운전 생략"},
}
...
def _month_component_arrays(
    population: Dict[str, Dict[str, np.ndarray]],
    household_index: int,
    trend_factor: float,
) -> Dict[str, np.ndarray]:
    calendar = month_calendar()
    arrays = {name: np.zeros(DAYS_IN_MONTH * 24, dtype=float) for name in ["fixed", "shift", "behavior", "hvac"]}
    for day_index, day_type in enumerate(calendar):
        sl = slice(day_index * 24, (day_index + 1) * 24)
        for name in arrays:
            arrays[name][sl] = population[day_type][name][household_index] * float(trend_factor)
    return arrays


def _move_shift_load(
    profile: np.ndarray,
    season: str,
    day_type: str,
    allowed: np.ndarray,
    shift_fraction: float,
    remote_availability: float,
) -> Tuple[np.ndarray, float]:
    result = np.asarray(profile, dtype=float).copy()
    allowed = np.asarray(allowed, dtype=bool)
    if result.sum() <= 1e-12 or not allowed.any():
        return result, 0.0
    weights = grid_weights(season, day_type).astype(float)
    movable = float(result[allowed].sum() * np.clip(shift_fraction * remote_availability, 0.0, 0.95))
    donor_score = np.where(allowed, result * np.maximum(weights - weights.min(), 0.0), 0.0)
    if donor_score.sum() <= 1e-12:
        donor_score = np.where(allowed, result, 0.0)
    movable = min(movable, float(result[donor_score > 0].sum()))
    if movable <= 1e-12:
        return result, 0.0
    donor_take = movable * donor_score / max(float(donor_score.sum()), 1e-12)
    donor_take = np.minimum(donor_take, result)
    actual_moved = float(donor_take.sum())
    result -= donor_take

    recipient_score = np.where(allowed & (weights == weights[allowed].min()), 1.0, 0.0)
    if recipient_score.sum() <= 1e-12:
        recipient_score = np.where(allowed, 1.0 / np.maximum(weights, 1.0), 0.0)
    result += actual_moved * recipient_score / max(float(recipient_score.sum()), 1e-12)
    return result, actual_moved
...
def _allocate_reduction(
    ...
    profile = np.asarray(component_profile, dtype=float)
    hours = np.arange(len(profile)) % 24
    preferred = np.isin(hours, np.asarray(preferred_hours, dtype=int))
    capacity = np.where(future_mask & preferred, profile * float(max_fraction), 0.0)
    if capacity.sum() <= 1e-12:
        capacity = np.where(future_mask, profile * float(max_fraction), 0.0)
    available = float(capacity.sum())
    applied = min(max(float(target_kwh), 0.0), available)
    if applied <= 1e-12:
        return np.zeros_like(profile), 0.0
    reduction = applied * capacity / max(available, 1e-12)
    return reduction, float(reduction.sum())


def build_direct_control_plan(
    household: pd.Series,
    household_index: int,
    population: Dict[str, Dict[str, np.ndarray]],
    ...
    """남은 기간의 직접제어 예상곡선을 생성합니다.

    가전별 실측분해값을 사용하지 않고, 가입 시 등록된 연결기기와 표준 프로파일을
    이용해 제어계획을 산정합니다. 시뮬레이션 내부 구성부하는 성능검증용입니다.
    """
    total_hours = DAYS_IN_MONTH * 24
    cutoff = int(np.clip(cutoff, 1, total_hours))
    future_mask = np.arange(total_hours) >= cutoff
    calendar = month_calendar()
    cfg = DIRECT_CONTROL_MODES[direct_mode]
    components = _month_component_arrays(population, household_index, trend_factor)

    no_control = np.asarray(actual_profile, dtype=float).copy()
    no_control[cutoff:] = np.asarray(baseline_profile, dtype=float)[cutoff:] * float(trend_factor)

    # 예약가전은 에너지를 보존하면서 저부하 시간으로 이동합니다.
    shifted_total = 0.0
    shifted_by_day = np.zeros(DAYS_IN_MONTH, dtype=float)
    for day_index, day_type in enumerate(calendar):
        sl = slice(day_index * 24, (day_index + 1) * 24)
        allowed = np.ones(24, dtype=bool)
        if day_index < cutoff // 24:
            continue
        if day_index == cutoff // 24:
            allowed[: cutoff % 24] = False
        new_shift, moved = _move_shift_load(
            components["shift"][sl], season, day_type, allowed,
            float(cfg["shift_fraction"]) if allow_shift else 0.0, float(household.get("원격제어가용도", 1.0)),
        )
        components["shift"][sl] = new_shift
        shifted_total += moved
        shifted_by_day[day_index] = moved

    action_rows: List[Dict[str, object]] = []
    action_reductions: Dict[str, np.ndarray] = {}
    delivery = float(cfg["delivery_factor"])
    if not package_table.empty:
        for _, row in package_table.iterrows():
            action_id = str(row.get("행동ID", ""))
            if action_id not in DIRECT_ACTION_META:
                continue
            meta = DIRECT_ACTION_META[action_id]
            planned = float(row["예상절감(kWh)"]) * delivery
            component_name = str(meta["component"])
            reduction, applied = _allocate_reduction(
                components[component_name], planned, future_mask,
                list(meta["hours"]), float(meta["max_fraction"]),
            )
            components[component_name] = np.maximum(components[component_name] - reduction, 0.0)
            action_reductions[action_id] = reduction
            wd = 0.0
            we = 0.0
            wd_count = 0
            we_count = 0
            for day_index, day_type in enumerate(calendar):
                if day_index < cutoff // 24:
                    continue
                day_value = float(reduction[day_index * 24:(day_index + 1) * 24].sum())
                if day_type == "주중":
                    wd += day_value
                    wd_count += 1
                else:
            ...
            action_rows.append({
                "가전·기능": meta["device"],
                "직접제어 방식": meta["method"],
                "남은기간 계획감축(kWh)": float(row["예상절감(kWh)"]),
                "기술성공률 반영감축(kWh)": applied,
                "주중 평균감축(kWh/일)": wd / max(wd_count, 1),
                "주말 평균감축(kWh/일)": we / max(we_count, 1),
                "고객 승인한도": f"{int(row['권고량'])}{row['단위']}",
            })

    controlled = no_control.copy()
    controlled_future = sum(components.values())
    controlled[cutoff:] = controlled_future[cutoff:]
    ...
    for day_index, day_type in enumerate(calendar):
        ...
        if day_index < cutoff // 24:
            status = "실제"
        elif day_index == cutoff // 24:
            status = "당일 관측+예측"
        else:
            status = "예측"
        before = float(no_control[sl].sum())
        after = float(controlled[sl].sum())
        daily_rows.append({
            "일자": day_index + 1,
            "대표일": day_type,
            "구분": status,
            "직접제어 미적용(kWh)": before,
            "한전 직접제어(kWh)": after,
            "예상감축(kWh)": before - after,
            "시간이동량(kWh)": shifted_by_day[day_index],
        })
    daily_table = pd.DataFrame(daily_rows)

    average_rows: List[Dict[str, object]] = []
    current_day_index = cutoff // 24
    for day_type in ["주중", "주말"]:
        indexes = [d for d, dt in enumerate(calendar) if d >= current_day_index and dt == day_type]
        if not indexes:
            continue
        before_profiles = []
        after_profiles = []
        for d in indexes:
            ...
            before_profiles.append(no_control[sl])
            after_profiles.append(controlled[sl])
        before_avg = np.mean(before_profiles, axis=0)
        after_avg = np.mean(after_profiles, axis=0)
        for hour in range(24):
            average_rows.append({
                "대표일": day_type,
                "시간": hour,
                "직접제어 미적용(kW)": float(before_avg[hour]),
                "한전 직접제어(kW)": float(after_avg[hour]),
                "차이(kW)": float(before_avg[hour] - after_avg[hour]),
            })
    average_table = pd.DataFrame(average_rows)

    if shifted_total > 1e-9:
        action_rows.append({
            "가전·기능": "세탁기·건조기·식기세척기 등 예약가전",
            "직접제어 방식": "고부하시간 운전을 저부하시간으로 이동",
            "남은기간 계획감축(kWh)": 0.0,
            "기술성공률 반영감축(kWh)": 0.0,
            "주중 평균감축(kWh/일)": 0.0,
            "주말 평균감축(kWh/일)": 0.0,
            "고객 승인한도": f"{shifted_total:.1f}kWh 시간 이동",
        })

    device_table = pd.DataFrame(action_rows)
    observed = float(actual_profile[:cutoff].sum())
    projected_usage = float(controlled.sum())
    no_control_usage = float(no_control.sum())
    return {
        "no_control": no_control,
        "controlled": controlled,
        "daily": daily_table,
        "average": average_table,
        "devices": device_table,
        "shifted_kwh": shifted_total,
        "reduced_kwh": max(no_control_usage - projected_usage, 0.0),
        "observed_kwh": observed,
        "no_control_month_kwh": no_control_usage,
        "controlled_month_kwh": projected_usage,
    }
...
def main() -> None:
    st.set_page_config(page_title="단일계량값 기반 100가구 직접제어 v7", page_icon="⚡", layout="wide")
    st.title("⚡ 단일계량값 기반 100가구 전력예산·알림·직접제어 시뮬레이터")
    st.caption(f"앱 버전 {APP_VERSION}")
    st.success(
        "현실성 개선: 고객별 가전기기 실측 사용량은 알고리즘에 제공하지 않습니다. 총계량값으로 월말 사용량·요금을 예측하고, 고객이 선택한 경우에만 사전 등록·연결된 스마트가전을 한전이 직접 제어하는 시나리오를 계산합니다. 가전별 자료는 시뮬레이션 생성·제어계획 산정·성능검증의 숨은 정답으로만 사용합니다."
    )
    ...
    with st.sidebar:
        ...
        run = st.button("단일계량값·직접제어 시뮬레이션 실행", type="primary", use_container_width=True)

    if not run and "meter_v7" not in st.session_state:
        ...
        st.markdown(
            ...
            - 고객이 선택하면 등록·연결된 스마트가전을 한전이 직접 제어하는 남은 기간 시나리오 제공
            - 직접제어 미적용 대비 월 누적곡선, 남은 일별 사용량, 주중·주말 평균 부하곡선과 가전별 감축계획 표시
            """
        )
        return
    ...
    result = st.session_state.get("meter_v7")
    if not result:
        return
    ...
    st.subheader("5. 고객 선택형 관리방식과 목표")
    service_mode = st.radio(
        "관리 방식",
        ["알림·행동권고", "한전 직접제어 위임"],
        horizontal=True,
        help="직접제어는 고객이 사전에 등록하고 통신 연결한 스마트가전에만 적용되며, 고객 수동해제와 안전·쾌적성 한도가 항상 우선합니다.",
    )
    goal_type = st.selectbox(
        "목표",
        ["현재 요금제 제공량 이내", "전월과 같은 요금", "전년 동월과 같은 요금", "사용자 지정 목표요금"],
        index=0,
        key="v7_goal_type",
    )
    custom_bill = st.number_input(
        "사용자 지정 목표요금(원)",
        min_value=int(fee),
        value=int(max(fee, mon["월말예상요금(원)"] - 10_000)),
        step=1_000,
        disabled=goal_type != "사용자 지정 목표요금",
        key="v7_custom_bill",
    )
    ...
    if unavoidable > 0:
        st.error(
            f"이미 목표사용량을 {unavoidable:.1f}kWh 초과해 이번 달에는 목표 달성이 불가능합니다. "
            "직접제어를 선택하더라도 이미 사용한 전력은 되돌릴 수 없으므로 예상 최저요금과 다음 달 요금제 전환을 함께 검토해야 합니다."
        )
    elif required_reduction <= 0.01:
        st.success("현재 예측상 별도의 감축 없이 목표를 달성할 수 있습니다. 직접제어를 선택하면 계통피크 완화를 위한 시간 이동만 수행할 수 있습니다.")
    else:
        st.warning(
    ...
    if service_mode == "한전 직접제어 위임":
        st.info(
            "직접제어는 단일 계량값으로 가전별 사용량을 추정해 임의 차단하는 방식이 아닙니다. "
            "고객이 가입 시 등록하고 통신 연결한 기기만, 승인한 운전횟수·온도·시간한도 안에서 제어합니다."
        )
        direct_mode = st.selectbox("직접제어 강도", list(DIRECT_CONTROL_MODES.keys()), index=1, key="v7_direct_mode")
        available_actions = candidates["행동대안"].tolist() if not candidates.empty else []
        authorized_actions = st.multiselect(
            "한전 직접제어를 허용할 항목",
            available_actions,
            default=available_actions,
            help="등록·연결되지 않은 기기는 실제 서비스에서 선택할 수 없습니다.",
            key="v7_authorized_actions",
        )
        allow_shift = st.checkbox("세탁기·건조기·식기세척기 등 예약가전 시간 이동 허용", value=True, key="v7_allow_shift")
        filtered_candidates = candidates[candidates["행동대안"].isin(authorized_actions)].copy() if available_actions else candidates.copy()
        style_name = str(DIRECT_CONTROL_MODES[direct_mode]["package_style"])
        direct_package, direct_metrics = optimize_action_package(required_reduction, filtered_candidates, style_name)
        direct_result = build_direct_control_plan(
            customer, idx, result["population"], actual[idx], baseline[idx], cutoff, season,
            float(mon["추세계수"]), direct_package, direct_mode, allow_shift=allow_shift,
        )
        direct_bill = subscription_bill(direct_result["controlled_month_kwh"], fee, quota, overage_rate)
        no_control_bill = subscription_bill(direct_result["no_control_month_kwh"], fee, quota, overage_rate)
        d1, d2, d3, d4 = st.columns(4)
        d1.metric("직접제어 미적용 예상", f"{direct_result['no_control_month_kwh']:,.1f} kWh")
        d2.metric("한전 직접제어 예상", f"{direct_result['controlled_month_kwh']:,.1f} kWh", f"-{direct_result['reduced_kwh']:,.1f} kWh")
        d3.metric("직접제어 예상요금", f"{direct_bill:,.0f}원", f"-{max(no_control_bill-direct_bill,0):,.0f}원")
        d4.metric("예약가전 시간이동", f"{direct_result['shifted_kwh']:,.1f} kWh")

        if direct_result["controlled_month_kwh"] <= target_usage + 0.5:
            st.success("현재 승인범위의 직접제어 계획으로 목표 달성이 가능한 것으로 추정됩니다.")
        else:
            gap = direct_result["controlled_month_kwh"] - target_usage
            st.warning(f"승인된 기기와 한도만으로는 목표보다 약 {gap:.1f}kWh 높게 예상됩니다. 목표조정, 추가 기기연결 또는 요금제 전환 검토가 필요합니다.")

    # 월 누적 그래프: 과거 실제와 미래 미적용/직접제어 시나리오를 한 화면에 표시
    ...
    observed_complete_days = max((cutoff - 1) // 24, 0)
    observed_daily = np.full(DAYS_IN_MONTH, np.nan)
    if observed_complete_days > 0:
        observed_daily[:observed_complete_days] = daily_actual[:observed_complete_days]
    current_day_index = (cutoff - 1) // 24
    observed_daily[current_day_index] = actual[idx, current_day_index * 24:cutoff].sum()
    observed_cum = np.nancumsum(np.nan_to_num(observed_daily, nan=0.0))
    observed_plot = np.where(np.arange(DAYS_IN_MONTH) <= current_day_index, observed_cum, np.nan)

    cum_fig = go.Figure()
    cum_fig.add_trace(go.Scatter(x=np.arange(1, DAYS_IN_MONTH + 1), y=observed_plot, mode="lines+markers", name="조회시점까지 실제 누적"))
    if direct_result is not None:
        no_daily = direct_result["daily"]["직접제어 미적용(kWh)"].to_numpy(dtype=float)
        controlled_daily = direct_result["daily"]["한전 직접제어(kWh)"].to_numpy(dtype=float)
        cum_fig.add_trace(go.Scatter(x=np.arange(1, DAYS_IN_MONTH + 1), y=np.cumsum(no_daily), mode="lines", name="직접제어 미적용 예상", line=dict(dash="dash")))
        cum_fig.add_trace(go.Scatter(x=np.arange(1, DAYS_IN_MONTH + 1), y=np.cumsum(controlled_daily), mode="lines", name="한전 직접제어 예상"))
    else:
        projected = actual[idx].copy()
        projected[cutoff:] = baseline[idx, cutoff:] * float(mon["추세계수"])
        cum_fig.add_trace(go.Scatter(x=np.arange(1, DAYS_IN_MONTH + 1), y=np.cumsum(projected.reshape(DAYS_IN_MONTH, 24).sum(axis=1)), mode="lines", name="현재 추세 예상", line=dict(dash="dash")))
    cum_fig.add_trace(go.Scatter(x=np.arange(1, DAYS_IN_MONTH + 1), y=day_cum_baseline, mode="lines", name="과거 기준패턴", line=dict(dash="dot")))
    cum_fig.add_hline(y=quota, line_dash="dot", annotation_text=f"{customer['현재요금제']} 제공량")
    cum_fig.update_layout(title="월 누적 총계량값과 직접제어 예상", xaxis_title="일자", yaxis_title="누적 kWh", height=420)
    ...
    day_fig.add_trace(go.Scatter(x=HOURS, y=forecast_values, mode="lines+markers", name="남은 시간 미제어 예측", line=dict(dash="dash")))
    day_fig.add_trace(go.Scatter(x=HOURS, y=day_base, mode="lines", name="과거 기준패턴", line=dict(dash="dot")))
    if direct_result is not None:
        direct_day = direct_result["controlled"][start:start + 24]
        direct_future = np.where(observed_mask, np.nan, direct_day)
        day_fig.add_trace(go.Scatter(x=HOURS, y=direct_future, mode="lines+markers", name="한전 직접제어 예상"))
    if st.checkbox("시뮬레이션 검증용 미래 실제값 표시", value=False, key="v7_hidden_actual"):
    ...
    if direct_result is not None:
        st.markdown("**남은 기간 일별 사용량 패턴**")
        future_daily = direct_result["daily"][direct_result["daily"]["일자"] >= int(result["current_day"])].copy()
        remaining_fig = go.Figure()
        remaining_fig.add_trace(go.Scatter(x=future_daily["일자"], y=future_daily["직접제어 미적용(kWh)"], mode="lines+markers", name="직접제어 미적용"))
        remaining_fig.add_trace(go.Scatter(x=future_daily["일자"], y=future_daily["한전 직접제어(kWh)"], mode="lines+markers", name="한전 직접제어"))
        remaining_fig.update_layout(title="남은 기간의 일별 예상사용량", xaxis_title="일자", yaxis_title="kWh/일", height=380)
        st.plotly_chart(remaining_fig, use_container_width=True)

        st.markdown("**주중·주말 평균 일일 부하곡선**")
        weekday_tab, weekend_tab = st.tabs(["주중 평균", "주말 평균"])
        for tab, day_type in [(weekday_tab, "주중"), (weekend_tab, "주말")]:
            with tab:
                avg = direct_result["average"]
                avg = avg[avg["대표일"] == day_type]
                if avg.empty:
                    st.info(f"남은 기간에 {day_type}이 없습니다.")
                else:
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=avg["시간"], y=avg["직접제어 미적용(kW)"], mode="lines+markers", name="직접제어 미적용"))
                    fig.add_trace(go.Scatter(x=avg["시간"], y=avg["한전 직접제어(kW)"], mode="lines+markers", name="한전 직접제어"))
                    fig.update_layout(title=f"남은 기간 {day_type} 평균 부하곡선", xaxis_title="시간", yaxis_title="kW", height=380)
                    st.plotly_chart(fig, use_container_width=True)

        st.markdown("**가전·기능별 직접제어 예정량**")
        if direct_result["devices"].empty:
            st.info("현재 목표에는 가전 사용량 감축이 필요하지 않으며, 승인된 경우 예약가전 시간 이동만 수행합니다.")
        else:
            st.dataframe(direct_result["devices"].round(2), use_container_width=True, hide_index=True)
        st.caption(
            "표의 가전별 수치는 개별 계량으로 식별한 실적이 아니라 고객이 등록·연결한 기기, 원본 표준부하, 현재 총계량 추세와 승인한 제어한도를 이용한 계획값입니다. 실제 운영에서는 기기 API/HEMS의 명령 성공 여부를 확인하고 고객 수동해제를 항상 허용해야 합니다."
        )
    ...
    st.subheader("6. 목표 달성을 위한 대안 또는 직접제어 계획")
    if service_mode == "알림·행동권고":
        ...
        package_tables["한전직접제어계획"] = direct_package
        direct_bill = subscription_bill(direct_result["controlled_month_kwh"], fee, quota, overage_rate) if direct_result is not None else float(mon["월말예상요금(원)"])
        direct_metrics_row = {
            "대안유형": f"한전 직접제어-{direct_mode}",
            **direct_metrics,
            "기술성공률반영감축(kWh)": direct_result["reduced_kwh"] if direct_result is not None else 0.0,
            "직접제어월말사용량(kWh)": direct_result["controlled_month_kwh"] if direct_result is not None else forecast_usage,
            "직접제어예상요금(원)": direct_bill,
            "시간이동량(kWh)": direct_result["shifted_kwh"] if direct_result is not None else 0.0,
        }
        package_metrics = pd.DataFrame([direct_metrics_row])
        st.dataframe(package_metrics.round(2), use_container_width=True, hide_index=True)

    st.caption(
        "행동권고는 고객 이행확률을 반영하며, 직접제어는 등록·연결된 기기의 기술성공률을 반영합니다. 두 방식 모두 고객의 수동해제, 안전조건, 냉난방 쾌적성 한도가 우선합니다."
    )
    ...
    st.subheader("7. 100가구 고객별 모니터링·요금제 추천")
    display_cols = [
    ...
    if direct_result is not None:
        download_tables["선택고객_직접제어_일별"] = direct_result["daily"]
        download_tables["선택고객_직접제어_주중주말평균"] = direct_result["average"]
        download_tables["선택고객_직접제어_가전별계획"] = direct_result["devices"]
    st.download_button(
        "결과자료 ZIP(CSV) 다운로드",
        data=make_zip(download_tables),
        file_name="100가구_단일계량값_전력예산_직접제어_v7.zip",
        mime="application/zip",
        use_container_width=True,
    )

    st.warning(
        "모형 한계: 가구별 시간단위 총계량값이 확보된다는 전제입니다. 직접제어는 고객이 등록하고 통신 연결한 스마트가전·HEMS가 있어야 하며, 단일 계량값만으로 특정 가전을 식별해 차단하는 기능은 아닙니다. 아파트 단지 전체 계량값만 있으면 세대별 서비스는 불가능합니다. 가전별 감축량과 기술성공률은 실증 전 추정값이므로 실제 AMI·기기응답 자료로 지속 보정해야 합니다."
    )
