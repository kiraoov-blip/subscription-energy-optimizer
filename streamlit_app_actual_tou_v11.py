...
APP_VERSION = "2026-07-20-actual-tou-v11.0"
...
def annual_bill_comparison(
    ...
    """월별 청구 구조를 유지하면서 12개월 요금을 합산합니다."""
    rows = []
    totals = {"일반 주택용": 0.0, "제주 TOU(3kW)": 0.0, "기본형": 0.0, "프리미엄형": 0.0}
    for _, r in customer_monthly.sort_values("월").iterrows():
        m = int(r["월"])
        u = float(r["총사용량_kWh"])
        if u > 0:
            off = float(r["경부하_kWh"]) / u
            mid = float(r["중간부하_kWh"]) / u
            peak = float(r["최대부하_kWh"]) / u
        else:
            off, mid, peak = 0.0, 0.0, 0.0
        totals["일반 주택용"] += residential_bill(u, m)
        totals["제주 TOU(3kW)"] += tou_bill(u, m, off, mid, peak, 3.0)
        totals["기본형"] += subscription_bill(u, basic_fee, basic_inc, overage)
        totals["프리미엄형"] += subscription_bill(u, premium_fee, premium_inc, overage)
    best = min(totals.values()) if totals else 0.0
    for name, value in totals.items():
        rows.append({"요금제": name, "연간요금(원)": value, "최저 대비 차이(원)": value - best})
    return pd.DataFrame(rows).sort_values("연간요금(원)").reset_index(drop=True)


def customer_annual_monthly_detail(
    customer_monthly: pd.DataFrame,
    basic_fee: float,
    basic_inc: float,
    premium_fee: float,
    ...
    for _, r in customer_monthly.sort_values("월").iterrows():
        ...
        if u > 0:
            ...
            off, mid, peak = 0.0, 0.0, 0.0
        bills = {
            "일반 주택용": residential_bill(u, m),
            "제주 TOU(3kW)": tou_bill(u, m, off, mid, peak, 3.0),
            "기본형": subscription_bill(u, basic_fee, basic_inc, overage),
            "프리미엄형": subscription_bill(u, premium_fee, premium_inc, overage),
        }
        rows.append({
            "월": m,
            "사용량(kWh)": u,
            ...
            "기본형(원)": bills["기본형"],
            "프리미엄형(원)": bills["프리미엄형"],
            "월별 추천요금제": min(bills, key=bills.get),
            "기본형 초과량(kWh)": max(u - basic_inc, 0.0),
            "프리미엄형 초과량(kWh)": max(u - premium_inc, 0.0),
        })
    return pd.DataFrame(rows)


def build_customer_annual_tariff_monitor(
    ...
    """전체 고객의 12개월 실제 사용량과 월별 요금을 합산해 연간 추천요금제를 계산합니다."""
    metric_lookup = metrics.set_index("비식별고객ID")
    rows = []
    for cid, g in monthly.groupby("비식별고객ID", sort=False):
        if cid not in metric_lookup.index:
            continue
        g = g.sort_values("월")
        mt = metric_lookup.loc[cid]
        comp = annual_bill_comparison(g, basic_fee, basic_inc, premium_fee, premium_inc, overage)
        bill_map = dict(zip(comp["요금제"], comp["연간요금(원)"]))
        recommended = str(comp.iloc[0]["요금제"])
        annual_usage = float(g["총사용량_kWh"].sum())
        max_idx = g["총사용량_kWh"].idxmax()
        max_row = g.loc[max_idx]
        off = float(g["경부하_kWh"].sum()) / max(annual_usage, 1e-9)
        mid = float(g["중간부하_kWh"].sum()) / max(annual_usage, 1e-9)
        peak = float(g["최대부하_kWh"].sum()) / max(annual_usage, 1e-9)
        rows.append({
            "비식별고객ID": cid,
            "군집": mt["군집"],
            "연간사용량(kWh)": annual_usage,
            "월평균사용량(kWh)": annual_usage / 12.0,
            "최대월사용량(kWh)": float(max_row["총사용량_kWh"]),
            "최대사용월": int(max_row["월"]),
            "경부하비중(%)": off * 100.0,
            "중간부하비중(%)": mid * 100.0,
            "최대부하비중(%)": peak * 100.0,
            "일반주택용(원)": bill_map["일반 주택용"],
            "제주TOU(원)": bill_map["제주 TOU(3kW)"],
            "기본형(원)": bill_map["기본형"],
            "프리미엄형(원)": bill_map["프리미엄형"],
            "연간추천요금제": recommended,
            "TOU대비예상절감(원)": max(bill_map["제주 TOU(3kW)"] - bill_map[recommended], 0.0),
            "기본형연간제공량사용률(%)": annual_usage / max(basic_inc * 12.0, 1e-9) * 100.0,
            "프리미엄형연간제공량사용률(%)": annual_usage / max(premium_inc * 12.0, 1e-9) * 100.0,
            "기본형초과개월": int((g["총사용량_kWh"] > basic_inc).sum()),
            "프리미엄형초과개월": int((g["총사용량_kWh"] > premium_inc).sum()),
        })
    return pd.DataFrame(rows)



def optimize_transformer_profile(
    base: np.ndarray,
    limit_ratio: float,
    participation: float,
) -> Dict[str, object]:
    """변압기 목표 운전한도에 맞춰 대표일 24시간 부하를 최적화합니다.

    `limit_ratio`는 제어 전 최대부하를 100%로 보았을 때의 가상 운전한도입니다.
    실제 변압기 정격(kVA), 역률, 열적 한도는 반영하지 않습니다.

    직접제어 참여율 100% 기준으로 시간대별 기준부하의 최대 14%는 시간이동,
    최대 6%는 실제 감축이 가능하다고 가정합니다. 참여율에 비례해 가용량이 줄어듭니다.
    """
    base = np.maximum(np.asarray(base, dtype=float), 0.0)
    n = len(base)
    if n == 0:
        return {
            "after": base.copy(), "limit": 0.0, "peak_before": 0.0, "peak_after": 0.0,
            "shifted": 0.0, "reduced": 0.0, "overload_before": 0.0,
            "overload_after": 0.0, "hours_before": 0, "hours_after": 0,
            "shift_out": np.zeros(0), "shift_in": np.zeros(0), "reduction": np.zeros(0),
            "status": "자료 없음", "max_shift_fraction": 0.0, "max_reduce_fraction": 0.0,
        }

    peak_before = float(base.max())
    ...
    if overload_before <= 1e-9 or participation <= 0.0:
        ...
        return {
            ...
            "status": status, "max_shift_fraction": 0.14 * participation,
            "max_reduce_fraction": 0.06 * participation,
        }

    scale = 1000  # 0.001kW 단위 정수화
    base_i = np.rint(base * scale).astype(int)
    limit_i = int(round(limit * scale))
    max_shift_fraction = 0.14 * float(participation)
    max_reduce_fraction = 0.06 * float(participation)

    peak_window = set(range(16, 22))
    offpeak_window = set(list(range(22, 24)) + list(range(0, 8)))
    source_hours = [h for h in range(n) if base[h] > limit + 1e-9 or h in peak_window]

    max_shift_i = np.zeros(n, dtype=int)
    max_reduce_i = np.zeros(n, dtype=int)
    headroom_i = np.maximum(limit_i - base_i, 0)
    for h in source_hours:
        # 목표한도 초과시간은 전체 가용유연성을, 그 외 피크시간은 절반만 허용합니다.
        factor = 1.0 if base[h] > limit + 1e-9 else 0.5
        max_shift_i[h] = int(round(base_i[h] * max_shift_fraction * factor))
        max_reduce_i[h] = int(round(base_i[h] * max_reduce_fraction * factor))
    ...
    for h in range(n):
        so = model.NewIntVar(0, int(max_shift_i[h]), f"shift_out_{h}")
        rd = model.NewIntVar(0, int(max_reduce_i[h]), f"reduce_{h}")
        si = model.NewIntVar(0, int(headroom_i[h]), f"shift_in_{h}")
        upper = max(int(base_i[h] + headroom_i[h]), int(base_i[h]), limit_i)
        af = model.NewIntVar(0, max(upper, 1), f"after_{h}")
        ov = model.NewIntVar(0, max(upper - limit_i, int(base_i[h]), 1), f"overload_{h}")
        model.Add(af == int(base_i[h]) - so - rd + si)
        model.Add(ov >= af - limit_i)
        shift_out_vars.append(so)
        reduce_vars.append(rd)
        shift_in_vars.append(si)
        after_vars.append(af)
        overload_vars.append(ov)

    # 시간이동은 에너지를 보존합니다.
    model.Add(sum(shift_out_vars) == sum(shift_in_vars))

    # 운전한도 위반을 최우선으로 줄이고, 그 다음 실제 감축보다 시간이동을 우선합니다.
    overload_penalty = 100_000
    reduction_penalty = 40
    shift_penalty = 8
    non_offpeak_sink_penalty = 4
    objective = overload_penalty * sum(overload_vars)
    objective += reduction_penalty * sum(reduce_vars)
    objective += shift_penalty * sum(shift_out_vars)
    objective += non_offpeak_sink_penalty * sum(
        shift_in_vars[h] for h in range(n) if h not in offpeak_window
    )
    ...
    if status_code not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        return {
            ...
            "overload_after": overload_before, "hours_before": hours_before, "hours_after": hours_before,
            "shift_out": np.zeros(n), "shift_in": np.zeros(n), "reduction": np.zeros(n),
            "status": "최적화 실패", "max_shift_fraction": max_shift_fraction,
            "max_reduce_fraction": max_reduce_fraction,
        }

    shift_out = np.array([solver.Value(v) for v in shift_out_vars], dtype=float) / scale
    reduction = np.array([solver.Value(v) for v in reduce_vars], dtype=float) / scale
    shift_in = np.array([solver.Value(v) for v in shift_in_vars], dtype=float) / scale
    after = np.array([solver.Value(v) for v in after_vars], dtype=float) / scale
    overload_after_arr = np.maximum(after - limit, 0.0)
    overload_after = float(overload_after_arr.sum())
    hours_after = int((overload_after_arr > 1e-6).sum())
    if hours_after == 0:
        status = "운전한도 충족"
    elif overload_after < overload_before - 1e-6:
        status = "과부하 완화·잔여 초과 존재"
    else:
        status = "유연성 부족"
    ...
    return {
        ...
        "shifted": float(shift_out.sum()),
        "reduced": float(reduction.sum()),
...
with TAB2:
    ...
    monitor_period=m1.selectbox(
        "분석 기간",["연간"]+list(range(1,13)),index=0,key="monitor_period",
        format_func=lambda x:"연간 전체" if x=="연간" else f"{x}월"
    )
    if monitor_period=="연간":
        monitor_cutoff=None
        m2.info("2025년 12개월 실제 실적을 합산")
        st.caption("연간 분석은 각 월의 누진·TOU·구독요금을 월별로 계산한 뒤 12개월을 합산합니다. 연간 총사용량에 요금구간을 한 번만 적용하지 않습니다.")
        monitor=build_customer_annual_tariff_monitor(monthly,metrics,basic_fee,basic_inc,premium_fee,premium_inc,overage)
        recommendation_col="연간추천요금제"
        sort_options=["TOU대비예상절감(원)","연간사용량(kWh)","제주TOU(원)","최대월사용량(kWh)"]
    else:
        monitor_month=int(monitor_period)
        ...
        recommendation_col="추천요금제"
        sort_options=["TOU대비예상절감(원)","월말예상(kWh)","제주TOU(원)","예측오차(%)"]
    f1,f2,f3=st.columns(3)
    plan_options=["전체"]+sorted(monitor[recommendation_col].dropna().unique().tolist())
    ...
    sort_key=f3.selectbox("정렬 기준",sort_options)
    shown=monitor.copy()
    if selected_plan!="전체": shown=shown[shown[recommendation_col]==selected_plan]
    ...
    r2.metric("기본형 추천",f"{(shown[recommendation_col]=='기본형').sum():,}명")
    r3.metric("프리미엄형 추천",f"{(shown[recommendation_col]=='프리미엄형').sum():,}명")
    r4.metric("TOU 대비 평균 절감",f"{shown['TOU대비예상절감(원)'].mean():,.0f}원" if len(shown) else "-")
    annual_columns={
        "연간사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),
        "월평균사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),
        "최대월사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),
        "경부하비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
        "중간부하비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
        "최대부하비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
        "기본형연간제공량사용률(%)":st.column_config.NumberColumn(format="%.1f%%"),
        "프리미엄형연간제공량사용률(%)":st.column_config.NumberColumn(format="%.1f%%"),
    }
    monthly_columns={
    ...
    st.dataframe(
        ...
        column_config={**common_columns, **(annual_columns if monitor_period=="연간" else monthly_columns)}
    )
    if monitor_period=="연간":
        file_name="2025년_연간_고객별_요금모니터링.csv"
    else:
...
with TAB3:
    ...
    with left:
        ...
        analysis_period=st.selectbox(
            "분석 기간",["연간"]+list(range(1,13)),index=0,
            format_func=lambda x:"연간 전체" if x=="연간" else f"{x}월"
        )
        if analysis_period!="연간":
            month=int(analysis_period)
    ...
    if analysis_period=="연간":
        cm=monthly[monthly["비식별고객ID"]==customer].sort_values("월")
        annual_comp=annual_bill_comparison(cm,basic_fee,basic_inc,premium_fee,premium_inc,overage)
        annual_detail=customer_annual_monthly_detail(cm,basic_fee,basic_inc,premium_fee,premium_inc,overage)
        annual_usage=float(cm["총사용량_kWh"].sum())
        max_month_row=cm.loc[cm["총사용량_kWh"].idxmax()]
        recommended=str(annual_comp.iloc[0]["요금제"])
        with right:
            st.subheader(f"{customer} · {metric['군집']} · 2025년 연간 종합")
            a,b,c,d=st.columns(4)
            a.metric("연간 사용량",f"{annual_usage:,.1f} kWh")
            b.metric("월평균 사용량",f"{annual_usage/12:,.1f} kWh")
            c.metric("최대 사용월",f"{int(max_month_row['월'])}월",f"{float(max_month_row['총사용량_kWh']):,.1f} kWh")
            d.metric("연간 비용상 추천",recommended)
            st.caption("연간 요금은 12개월 각각의 요금을 계산해 합산했습니다. 기본 제공량과 누진구간도 매월 새로 적용됩니다.")
            st.dataframe(annual_comp,hide_index=True,use_container_width=True,column_config={"연간요금(원)":st.column_config.NumberColumn(format="₩%,.0f"),"최저 대비 차이(원)":st.column_config.NumberColumn(format="₩%,.0f")})

        st.subheader("월별 사용량과 구독 제공량")
        fig=go.Figure()
        fig.add_trace(go.Scatter(x=annual_detail["월"],y=annual_detail["사용량(kWh)"],name="실제 월사용량",mode="lines+markers"))
        fig.add_hline(y=basic_inc,line_dash="dash",annotation_text="기본형 제공량")
        fig.add_hline(y=premium_inc,line_dash="dot",annotation_text="프리미엄형 제공량")
        fig.update_layout(height=430,xaxis_title="월",yaxis_title="사용량(kWh)",xaxis={"dtick":1})
        st.plotly_chart(fig,use_container_width=True)

        st.subheader("월별 요금제 비교와 추천")
        st.dataframe(
            annual_detail,hide_index=True,use_container_width=True,height=500,
            column_config={
                "사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),
                "경부하 비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
                "중간부하 비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
                "최대부하 비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
                "일반주택용(원)":st.column_config.NumberColumn(format="₩%,.0f"),
                "제주TOU(원)":st.column_config.NumberColumn(format="₩%,.0f"),
                "기본형(원)":st.column_config.NumberColumn(format="₩%,.0f"),
                "프리미엄형(원)":st.column_config.NumberColumn(format="₩%,.0f"),
                "기본형 초과량(kWh)":st.column_config.NumberColumn(format="%.1f"),
                "프리미엄형 초과량(kWh)":st.column_config.NumberColumn(format="%.1f"),
            }
        )
        y1,y2,y3=st.columns(3)
        y1.metric("기본형 초과 월",f"{(annual_detail['기본형 초과량(kWh)']>0).sum()}개월")
        y2.metric("프리미엄형 초과 월",f"{(annual_detail['프리미엄형 초과량(kWh)']>0).sum()}개월")
        y3.metric("TOU 대비 최대 연간절감",f"{max(float(annual_comp.loc[annual_comp['요금제']=='제주 TOU(3kW)','연간요금(원)'].iloc[0])-float(annual_comp['연간요금(원)'].min()),0):,.0f}원")

        st.subheader("계절별 주중·주말 평균 시간대 패턴")
        annual_season=st.selectbox("대표 계절",["봄가을","여름","겨울"],index=1,key="annual_customer_season")
        prof=profiles[(profiles["비식별고객ID"]==customer)&(profiles["계절"]==annual_season)]
        t1,t2=st.tabs(["주중","주말"])
        for tab,dt in [(t1,"주중"),(t2,"주말")]:
            with tab:
                p=prof[prof["일유형"]==dt].sort_values("시간")
                fig=go.Figure(go.Scatter(x=p["시간대"],y=p["평균사용량_kWh"],name="실제 평균",mode="lines+markers"))
                fig.update_layout(height=390,xaxis_title="시간",yaxis_title="평균 사용량(kWh)",xaxis={"tickangle":-45})
                st.plotly_chart(fig,use_container_width=True)
                c1,c2=st.columns(2)
                c1.metric("평균 일사용량",f"{p['평균사용량_kWh'].sum():.2f} kWh")
                c2.metric("최대 시간사용량",f"{p['평균사용량_kWh'].max():.2f} kW")

        st.info("알림·직접제어는 월별 제공량과 월별 청구주기를 기준으로 작동하므로, 연간 화면에서는 실적·요금·계절 패턴을 종합 진단하고 실제 제어 시뮬레이션은 월별 화면에서 수행합니다.")
        files={
            f"{customer}_2025연간_요금비교.csv":annual_comp.to_csv(index=False).encode("utf-8-sig"),
            f"{customer}_2025월별_상세.csv":annual_detail.to_csv(index=False).encode("utf-8-sig"),
        }
        st.download_button("선택 고객 연간 결과 ZIP 다운로드",zip_results(files),file_name=f"{customer}_2025연간_진단결과.zip",mime="application/zip")
    else:
...
with TAB4:
    ...
    portfolio_period=p2.selectbox(
        "분석 기간",["연간"]+list(range(1,13)),index=0,key="portfolio_period",
        format_func=lambda x:"연간 전체" if x=="연간" else f"{x}월"
    )
    if portfolio_period=="연간":
        p3.info("2025년 12개월 실적")
        pcut=None
    else:
        pmonth=int(portfolio_period)
    ...
    transformer_limit_ratio=st.slider(
        "변압기 목표 운전한도(제어 전 최대부하 대비)",
        60,110,90,1,format="%d%%",key="transformer_limit_ratio",
        help="100가구 대표일의 제어 전 최대부하를 100%로 두고 가상 운전한도를 설정합니다. 실제 변압기 정격(kVA)은 아닙니다."
    )/100.0
    st.caption("예: 90%를 선택하면 제어 전 최대부하의 90% 이하를 목표로 부하이동·감축을 배분합니다. 가용 유연성이 부족하면 잔여 초과부하가 표시됩니다.")
    sample=metrics.sample(n=100,random_state=int(seed))
    sd=daily[daily["비식별고객ID"].isin(sample["비식별고객ID"])]

    if portfolio_period=="연간":
        pf=build_customer_annual_tariff_monitor(monthly[monthly["비식별고객ID"].isin(sample["비식별고객ID"])],sample,basic_fee,basic_inc,premium_fee,premium_inc,overage)
        recommendation_col="연간추천요금제"
        k1,k2,k3,k4=st.columns(4)
        k1.metric("표본 고객",f"{len(pf)}명")
        k2.metric("연간 사용량 합계",f"{pf['연간사용량(kWh)'].sum():,.0f} kWh")
        k3.metric("기본형 추천",f"{(pf[recommendation_col]=='기본형').sum()}명")
        k4.metric("프리미엄형 추천",f"{(pf[recommendation_col]=='프리미엄형').sum()}명")
        ...
        st.subheader("100가구 월별 합계 사용량")
        pm=monthly[monthly["비식별고객ID"].isin(sample["비식별고객ID"])].groupby("월",as_index=False)["총사용량_kWh"].sum()
        fig=go.Figure(go.Scatter(x=pm["월"],y=pm["총사용량_kWh"],name="월별 합계",mode="lines+markers"))
        fig.update_layout(height=400,xaxis_title="월",yaxis_title="100가구 합계 사용량(kWh)",xaxis={"dtick":1})
        st.plotly_chart(fig,use_container_width=True)
        pseason=st.selectbox("대표 부하곡선 계절",["봄가을","여름","겨울"],index=1,key="portfolio_season")
    else:
        forecast_rows=[]
        ...
        recommendation_col="추천요금제"
        k1,k2,k3,k4=st.columns(4);k1.metric("표본 고객",f"{len(pf)}명");k2.metric("월말 예상합계",f"{pf['월말예상(kWh)'].sum():,.0f} kWh");k3.metric("기본형 추천",f"{(pf[recommendation_col]=='기본형').sum()}명");k4.metric("프리미엄형 추천",f"{(pf[recommendation_col]=='프리미엄형').sum()}명")
        cdist=pf["군집"].value_counts().reset_index();cdist.columns=["군집","고객수"];cdist["비중(%)"]=cdist["고객수"]/len(pf)*100.0
        st.dataframe(cdist,hide_index=True,use_container_width=True,column_config={"비중(%)":st.column_config.NumberColumn(format="%.1f%%")})
        st.bar_chart(cdist.set_index("군집")[["고객수"]])
        pseason=season_for_month(pmonth)

    sprof=profiles[(profiles["비식별고객ID"].isin(sample["비식별고객ID"]))&(profiles["계절"]==pseason)]
    agg=sprof.groupby(["일유형","시간","시간대"],as_index=False)["평균사용량_kWh"].sum()
    dt=st.radio("포트폴리오 부하곡선",["주중","주말"],horizontal=True)
    ap=agg[agg["일유형"]==dt].sort_values("시간")
    base=ap["평균사용량_kWh"].to_numpy(float)
    transformer=optimize_transformer_profile(base,transformer_limit_ratio,participation)
    after=np.asarray(transformer["after"],dtype=float)

    st.subheader("변압기 목표 운전한도 기반 직접제어")
    fig=go.Figure()
    fig.add_trace(go.Scatter(x=ap["시간대"],y=base,name="제어 전 합계부하",mode="lines+markers"))
    fig.add_trace(go.Scatter(x=ap["시간대"],y=after,name="제어 후 합계부하",mode="lines+markers"))
    fig.add_hline(y=transformer["limit"],line_dash="dash",line_color="red",annotation_text=f"목표 운전한도 {transformer['limit']:.1f}kW")
    fig.update_layout(height=460,xaxis_title="시간",yaxis_title="100가구 합계부하(kW)",xaxis={"tickangle":-45})
    st.plotly_chart(fig,use_container_width=True)

    q1,q2,q3,q4=st.columns(4)
    q1.metric("제어 전 최대부하",f"{transformer['peak_before']:.1f} kW")
    q2.metric("설정 운전한도",f"{transformer['limit']:.1f} kW",f"최대부하의 {transformer_limit_ratio:.0%}")
    q3.metric("제어 후 최대부하",f"{transformer['peak_after']:.1f} kW",f"{transformer['peak_after']-transformer['peak_before']:.1f} kW")
    q4.metric("제어 결과",transformer["status"])
    r1,r2,r3,r4=st.columns(4)
    r1.metric("한도 초과시간",f"{transformer['hours_before']} → {transformer['hours_after']}시간")
    r2.metric("초과부하량",f"{transformer['overload_before']:.1f} → {transformer['overload_after']:.1f} kWh")
    r3.metric("시간이동량",f"{transformer['shifted']:.1f} kWh")
    r4.metric("실제 감축량",f"{transformer['reduced']:.1f} kWh")

    if transformer["hours_after"]==0:
        st.success("선택한 참여율과 가정된 유연성 범위에서 설정한 변압기 운전한도를 충족했습니다.")
    elif transformer["overload_after"] < transformer["overload_before"]:
        st.warning("부하를 완화했으나 설정한 운전한도를 완전히 충족하지 못했습니다. 직접제어 참여율을 높이거나 운전한도를 조정해야 합니다.")
    else:
        st.error("현재 직접제어 참여율로는 유효한 과부하 완화가 어렵습니다.")
    st.caption(
        f"모형 가정: 직접제어 참여율 100% 기준 시간대 부하의 최대 14% 시간이동·6% 실제감축 가능. "
        f"현재 참여율에서는 각각 최대 {transformer['max_shift_fraction']:.1%}, {transformer['max_reduce_fraction']:.1%}를 적용합니다. "
        "실제 변압기 정격(kVA), 역률, 온도 및 고객-변압기 연결정보는 반영하지 않은 시나리오입니다."
    )

    transformer_detail=pd.DataFrame({
        "시간대":ap["시간대"].tolist(),
        "제어전부하(kW)":base,
        "시간이동출력(kW)":np.asarray(transformer["shift_out"]),
        "실제감축출력(kW)":np.asarray(transformer["reduction"]),
        "이동유입출력(kW)":np.asarray(transformer["shift_in"]),
        "제어후부하(kW)":after,
        "운전한도(kW)":float(transformer["limit"]),
        "잔여초과(kW)":np.maximum(after-float(transformer["limit"]),0.0),
    })
    with st.expander("시간대별 변압기 제어 상세표"):
        st.dataframe(
            transformer_detail,hide_index=True,use_container_width=True,
            column_config={c:st.column_config.NumberColumn(format="%.3f") for c in transformer_detail.columns if c!="시간대"}
        )
        st.download_button(
            "변압기 제어 상세 CSV 다운로드",
            transformer_detail.to_csv(index=False).encode("utf-8-sig"),
            file_name=f"실제100가구_{pseason}_{dt}_변압기제어상세.csv",mime="text/csv"
        )
    st.subheader("100가구 고객별 요금 모니터링 및 추천")
    annual_cfg={
        "연간사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),"월평균사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),"최대월사용량(kWh)":st.column_config.NumberColumn(format="%.1f"),
        "경부하비중(%)":st.column_config.NumberColumn(format="%.1f%%"),"중간부하비중(%)":st.column_config.NumberColumn(format="%.1f%%"),"최대부하비중(%)":st.column_config.NumberColumn(format="%.1f%%"),
        "기본형연간제공량사용률(%)":st.column_config.NumberColumn(format="%.1f%%"),"프리미엄형연간제공량사용률(%)":st.column_config.NumberColumn(format="%.1f%%"),
    }
    monthly_cfg={"현재누적(kWh)":st.column_config.NumberColumn(format="%.1f"),"월말예상(kWh)":st.column_config.NumberColumn(format="%.1f"),"실제월사용량(kWh)":st.column_config.NumberColumn(format="%.1f")}
    money_cfg={"일반주택용(원)":st.column_config.NumberColumn(format="₩%,.0f"),"제주TOU(원)":st.column_config.NumberColumn(format="₩%,.0f"),"기본형(원)":st.column_config.NumberColumn(format="₩%,.0f"),"프리미엄형(원)":st.column_config.NumberColumn(format="₩%,.0f"),"TOU대비예상절감(원)":st.column_config.NumberColumn(format="₩%,.0f")}
    sort_col="연간사용량(kWh)" if portfolio_period=="연간" else "월말예상(kWh)"
    st.dataframe(pf.sort_values(sort_col,ascending=False),hide_index=True,use_container_width=True,column_config={**money_cfg,**(annual_cfg if portfolio_period=="연간" else monthly_cfg)})
    filename="실제100가구_연간_요금모니터링_추천.csv" if portfolio_period=="연간" else "실제100가구_월별_요금모니터링_추천.csv"
    st.download_button("100가구 결과 CSV 다운로드",pf.to_csv(index=False).encode("utf-8-sig"),file_name=filename,mime="text/csv")
...
- 전체 914명을 대상으로 월별 모니터링뿐 아니라 **2025년 연간 사용량·12개월 요금합계·연간 추천요금제**를 한 표로 제공합니다.
- 고객별 진단과 100가구 포트폴리오에서도 `연간 전체`를 선택할 수 있으며, 월별 사용량·월별 추천·계절별 부하곡선을 함께 확인할 수 있습니다.
- 100가구 포트폴리오에서 **변압기 목표 운전한도를 제어 전 최대부하의 60~110%**로 직접 설정하고, 운전한도 초과시간·잔여 초과부하·시간이동량·실제 감축량을 확인할 수 있습니다.
...
4. **변압기 한도의 성격**: 사용자가 설정하는 비율은 대표 고객군의 제어 전 최대부하에 대한 가상 운전목표이며, 실제 변압기 정격(kVA)·역률·열적 한도를 의미하지 않습니다.
