...
APP_VERSION = "2026-07-20-actual-tou-v14.0"
...
with T2:
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v14_{year}_{period}_요금모니터링.csv","text/csv")
...
with T3:
    ...
    if period=="연간 종합진단":
        ...
        c[3].metric("제어 후 예상",fmt_kwh(max(f["forecast"]-effective,f["current"])))
        if not plan.empty:
            ps=round_table(plan)
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("수요관리우선점수",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v14_100가구_포트폴리오.zip","application/zip")
