...
APP_VERSION = "2026-07-20-actual-tou-v18.0"
...
# 일반 주택용과 제주 TOU에는 연료비조정액·기후환경요금·부가세·전력기금을 별도 반영함.
...
def round_half_up(value: float) -> int:
    """원 미만 4사5입을 위한 양수 기준 반올림."""
    return int(math.floor(float(value) + 0.5))


def finalize_electric_bill(base_and_energy: float, kwh: float) -> float:
    """기본·전력량요금에 연료비·기후환경요금, 부가세, 전력기금을 반영한 최종 청구액."""
    usage = max(float(kwh), 0.0)
    electricity_charge = (
        max(float(base_and_energy), 0.0)
        + usage * float(FUEL_ADJUSTMENT_RATE)
        + usage * float(CLIMATE_ENV_RATE)
    )
    vat = round_half_up(electricity_charge * float(VAT_RATE))
    # 전력산업기반기금은 10원 미만 절사
    fund = int(math.floor(electricity_charge * float(POWER_FUND_RATE) / 10.0) * 10)
    return float(round_half_up(electricity_charge + vat + fund))


def bill_component_breakdown(base_and_energy: float, kwh: float) -> Dict[str, float]:
    """화면 설명용 요금 구성요소."""
    usage = max(float(kwh), 0.0)
    fuel = usage * float(FUEL_ADJUSTMENT_RATE)
    climate = usage * float(CLIMATE_ENV_RATE)
    electricity_charge = max(float(base_and_energy), 0.0) + fuel + climate
    vat = round_half_up(electricity_charge * float(VAT_RATE))
    fund = int(math.floor(electricity_charge * float(POWER_FUND_RATE) / 10.0) * 10)
    total = round_half_up(electricity_charge + vat + fund)
    return {
        "기본·전력량·구독요금(원)": float(base_and_energy),
        "연료비조정액(원)": float(fuel),
...
def tou_bill(kwh: float, month: int, off_share: float, mid_share: float, peak_share: float, contract_kw: float=3.0) -> float:
    ...
    return finalize_electric_bill(4310*contract_kw+energy, kwh)
...
PLAN_ORDER = ["일반 주택용", "제주 TOU(3kW)", "구독 기본형", "구독 프리미엄형"]
PLAN_BILL_COLUMNS = {
    "일반 주택용": "일반주택용(원)",
    "제주 TOU(3kW)": "제주TOU(원)",
    "구독 기본형": "기본형(원)",
...
def dynamic_tariff_analysis(monthly: pd.DataFrame, basic_fee: float, basic_inc: float,
                            ...
                            fuel_rate: float, climate_rate: float, vat_rate: float, fund_rate: float) -> Dict[str, pd.DataFrame | float]:
    ...
    for _,r in monthly.iterrows():
        ...
        monthly_rows.append({
            ...
            "일반주택용(원)":bills["일반 주택용"],"제주TOU(원)":bills["제주 TOU(3kW)"],
...
st.set_page_config(page_title="제주 TOU 고객 전기요금 시뮬레이터", page_icon="⚡", layout="wide")
st.title("제주 TOU 고객 전기요금 모니터링·사용량 제어 시뮬레이터")
st.caption(f"앱 버전 {APP_VERSION} · 2024~2025년 공통 핵심고객 712명 · 일반·TOU는 부가요금·세금 별도 반영, 구독료는 모두 포함된 최종가격 · 요금은 원 단위 반올림")
...
with st.sidebar:
    ...
    st.info("일반 주택용과 제주 TOU에는 연료비조정액·기후환경요금·부가가치세·전력산업기반기금을 별도 반영합니다. 구독 기본형·프리미엄형은 표시된 월 구독료와 초과단가가 모든 부가요금·세금을 포함한 최종 소비자가격입니다.")
    st.caption("2026년 6월 요금표를 2024·2025년 사용량에 동일 적용한 비교 시나리오입니다.")
...
with st.spinner("현재 요금 설정으로 월별·연간 추천요금제를 다시 계산하고 있습니다..."):
    tariff_dynamic=dynamic_tariff_analysis(D["monthly"],basic_fee,basic_inc,premium_fee,premium_inc,overage,
                                                   fuel_rate,climate_rate,VAT_RATE,POWER_FUND_RATE)
...
with T1:
    ...
    c[4].metric("연간 추천요금제 유지",fmt_pct(tariff_dynamic["annual_stability"]))
...
with T2:
    ...
    if not monitor.empty:
        ...
        c1.metric("표시 고객",f"{len(show):,}명")
        c2.metric("일반주택용 추천",f"{(show['추천요금제']=='일반 주택용').sum():,}명")
        c3.metric("제주 TOU 추천",f"{(show['추천요금제']=='제주 TOU(3kW)').sum():,}명")
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v17_{year}_{period}_요금모니터링.csv","text/csv")
...
with T3:
    ...
    if period=="연간 종합진단":
        ...
        a,b,c=st.columns(3)
        plan_choices=["일반 주택용","제주 TOU(3kW)","구독 기본형","구독 프리미엄형"]
        ...
        st.dataframe(comparison,hide_index=True,use_container_width=True,column_config=dataframe_config(comparison))
        ...
        if not plan.empty:
            ps=round_table(plan)
            st.dataframe(ps,hide_index=True,use_container_width=True,column_config=dataframe_config(ps))
...
with T4:
    ...
    cs=cluster_summary.copy();cs["연도"]=cs["연도"].astype(int).astype(str);cs["비중(%)"]=cs.pop("비중")*100;cs=round_table(cs);st.dataframe(cs.sort_values(["연도","고객수"],ascending=[True,False]),hide_index=True,use_container_width=True,column_config=dataframe_config(cs))
...
with T5:
    ...
    st.markdown("### 1. 연간 추천요금제")
    annual_summary=round_table(tariff_dynamic["annual_summary"].copy())
    ...
    c1.metric("일반주택용 추천",f"{int(counts['일반 주택용']):,}명")
    c2.metric("제주 TOU 추천",f"{int(counts['제주 TOU(3kW)']):,}명")
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v17_{annual_year}_연간추천요금제.csv","text/csv")

    st.markdown("#### 연간 추천요금제 전이")
    ...
    m1.metric("일반주택용 추천",f"{int(mcounts['일반 주택용']):,}명")
    m2.metric("제주 TOU 추천",f"{int(mcounts['제주 TOU(3kW)']):,}명")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v17_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("수요관리우선점수",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v18_100가구_포트폴리오.zip","application/zip")

with T7:
    st.markdown("""
### v18 핵심 변경점
- 연료비조정단가, 기후환경요금단가, 부가가치세율, 전력산업기반기금 요율은 일반 주택용·제주 TOU에 동적으로 반영합니다.
- 고객별 진단·제어의 현재 적용 요금제에 **일반 주택용·제주 TOU·구독 기본형·구독 프리미엄형**을 모두 포함했습니다.
- 고객별 요금 모니터링에 4개 요금제별 추천 고객 수를 함께 표시합니다.
- 요금·예측 탭에 **월별 추천과 연간 추천**을 함께 제공하며, 연간 추천은 12개월 요금을 합산하여 산정합니다.
- 기본형·프리미엄형 구독료·제공량·초과단가가 변경되면 추천 고객 수와 요금요약을 즉시 재계산합니다.
- 요금요약에 고객당 평균·중앙 연간요금과 함께 고객당 평균·중앙 월요금을 추가했습니다.
...
- 목표 미달 시 잔여 격차를 표시하고, 관리 후 사용량 기준의 최적 요금제와 구독서비스 전환·제어범위 확대 등 대안을 안내합니다.
- 연도는 모든 표와 그래프에서 **2024, 2025**의 범주형 문자로 표시합니다.
- 고객 진단의 점수는 **사용패턴 일관성**과 **피크관리 필요도**라는 쉬운 등급으로 함께 설명합니다.
- 구독 기본형 **84,900원·450kWh**와 프리미엄형 **249,000원·1,000kWh**는 연료비조정액·기후환경요금·부가가치세·전력산업기반기금이 모두 포함된 월 최종 납부액으로 해석합니다.
- 제공량 이내에서는 구독료가 고정되며, 초과사용량에는 설정한 최종 초과단가만 추가합니다. 구독료에 부가요금이나 세금을 중복 가산하지 않습니다.
- 사용량·부하·점수는 소수점 첫째자리까지, **모든 요금·절감액은 원 단위 정수**로 표시합니다. 비중·증감률·예측오차·사용률은 % 단위로 표시합니다.
