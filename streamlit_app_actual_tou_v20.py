...
APP_VERSION = "2026-07-21-actual-tou-v20.0"
...
st.title("제주 TOU 공통고객 2개년 종단분석·요금 모니터링·맞춤제어 시뮬레이터")
...
with T2:
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v20_{year}_{period}_요금모니터링.csv","text/csv")
...
with T4:
    ...
    cs=cluster_summary.copy();cs["연도"]=cs["연도"].astype(int).astype(str);cs["비중(%)"]=cs.pop("비중")*100
    cs=cs.rename(columns={
        "연간사용량_kWh":"연간 사용량(kWh)",
        "최대시간사용량_kWh":"최대시간 사용량(kWh)",
        "주말주중비":"주중 대비 주말 사용량 비중",
    })
    cs=round_table(cs);st.dataframe(cs.sort_values(["연도","고객수"],ascending=[True,False]),hide_index=True,use_container_width=True,column_config=dataframe_config(cs))
...
with T5:
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v20_{annual_year}_연간추천요금제.csv","text/csv")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v20_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("2025년 연간 사용량(kWh)",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v20_100가구_분석결과.zip","application/zip")

with T7:
    st.markdown("""
### v20 핵심 변경점
- 탭·지표·표 제목을 내부 보고용으로 이해하기 쉬운 표현으로 정비했습니다.
- 월 표시를 1~12의 정수형으로 통일하고, 사용량·군집·요금추천 관련 열 이름을 명확하게 변경했습니다.
- 100가구 표에서 수요관리 우선점수 열을 제외하고 연도별 사용량·군집 유형 중심으로 정리했습니다.
