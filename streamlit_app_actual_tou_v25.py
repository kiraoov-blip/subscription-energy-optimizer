...
APP_VERSION = "2026-07-21-actual-tou-v25.0"
...
with T2:
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v25_{year}_{period}_요금모니터링.csv","text/csv")
...
with T5:
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v25_{annual_year}_연간추천요금제.csv","text/csv")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v25_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("2025년 연간 사용량(kWh)",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v25_100가구_분석결과.zip","application/zip")
