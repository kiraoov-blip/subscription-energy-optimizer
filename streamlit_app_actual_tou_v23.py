...
APP_VERSION = "2026-07-21-actual-tou-v23.0"
...
/* iPad Safari: 슬라이더 현재값(예: 90%, 20일)이 한 글자 폭으로 축소되는 현상 방지 */
...
with T2:
    ...
    cutoff=d.slider("조회일",5,maxday-1,min(20,maxday-1),1,format="%d일",disabled=period=="연간 전체",key="monitor_cutoff")
    if period!="연간 전체":
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v23_{year}_{period}_요금모니터링.csv","text/csv")
...
with T3:
    ...
    if period=="연간 종합진단":
        ...
        cutoff=c.slider("조회일",5,monthrange(year,month)[1]-1,min(20,monthrange(year,month)[1]-1),1,format="%d일",key="diag_cutoff")
...
with T5:
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v23_{annual_year}_연간추천요금제.csv","text/csv")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v23_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    a,b=st.columns(2)
    limit_pct=a.slider("변압기 목표 운전한도(제어 전 최대부하 대비)",60,110,90,1,format="%d%%",key="transformer_limit_pct")
    participation_pct=b.slider("직접제어 참여율",0,100,70,5,format="%d%%",key="direct_control_participation_pct")
    a.caption(f"현재 설정: {limit_pct}%")
    b.caption(f"현재 설정: {participation_pct}%")
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("2025년 연간 사용량(kWh)",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v23_100가구_분석결과.zip","application/zip")
