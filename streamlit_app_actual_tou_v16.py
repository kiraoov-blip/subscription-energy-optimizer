...
APP_VERSION = "2026-07-20-actual-tou-v16.0"
...
def dynamic_tariff_analysis(monthly: pd.DataFrame, basic_fee: float, basic_inc: float,
                            premium_fee: float, premium_inc: float, overage: float) -> Dict[str, pd.DataFrame | float]:
...
def inverse_bill_for_plan(target_bill: float, plan: str, month: int, monthly_row: pd.Series, basic_fee: float,
    ...
    if plan == "구독 기본형":
        return inverse_subscription_bill(target_bill, basic_fee, basic_inc, overage)
    if plan == "구독 프리미엄형":
        return inverse_subscription_bill(target_bill, premium_fee, premium_inc, overage)
    lo, hi = 0.0, 2_000.0
...
st.caption(f"앱 버전 {APP_VERSION} · 2024~2025년 공통 핵심고객 712명 · 사용량·부하는 소수점 첫째자리, 요금은 원 단위 반올림")
...
with T2:
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v16_{year}_{period}_요금모니터링.csv","text/csv")
...
with T5:
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v16_{annual_year}_연간추천요금제.csv","text/csv")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v16_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("수요관리우선점수",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v16_100가구_포트폴리오.zip","application/zip")
