...
APP_VERSION = "2026-07-21-actual-tou-v22.0"
...
with T2:
    ...
    maxday=monthrange(int(year),int(month))[1];cutoff=d.slider("조회일",5,maxday-1,min(20,maxday-1),1,format="%d일",disabled=period=="연간 전체")
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v22_{year}_{period}_요금모니터링.csv","text/csv")
...
with T5:
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v22_{annual_year}_연간추천요금제.csv","text/csv")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v22_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    a,b=st.columns(2);limit_pct=a.slider("변압기 목표 운전한도(제어 전 최대부하 대비)",60,110,90,1,format="%d%%");participation_pct=b.slider("직접제어 참여율",0,100,70,5,format="%d%%");base=base24 if target_year==2024 else base25;result=optimize_transformer_profile(base,limit_pct/100,participation_pct/100)
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("2025년 연간 사용량(kWh)",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v22_100가구_분석결과.zip","application/zip")
