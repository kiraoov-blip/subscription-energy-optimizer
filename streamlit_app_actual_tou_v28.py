...
APP_VERSION = "2026-07-21-actual-tou-v28.0"
...
st.set_page_config(page_title="J-CARE", page_icon="⚡", layout="wide")
st.markdown("""
<style>
/* J-CARE 첫 화면 네이밍 */
.jcare-title-block {
...
<div class="jcare-title-block">
    <div class="jcare-brand">J-CARE</div>
    <div class="jcare-korean">제주 TOU 고객 맞춤형 요금·에너지관리 시뮬레이터</div>
    <div class="jcare-english">Jeju Customer Analytics, Rate Recommendation &amp; Energy Management Simulator</div>
</div>
""", unsafe_allow_html=True)
...
with T2:
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v28_{year}_{period}_요금모니터링.csv","text/csv")
...
with T5:
    ...
    st.download_button("연간 추천요금제 고객표 CSV",annual_show.to_csv(index=False).encode("utf-8-sig"),f"v28_{annual_year}_연간추천요금제.csv","text/csv")
    ...
    st.download_button("월별 추천요금제 고객표 CSV",monthly_show.to_csv(index=False).encode("utf-8-sig"),f"v28_{monthly_year}_{monthly_month}월_추천요금제.csv","text/csv")
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("2025년 연간 사용량(kWh)",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v28_100가구_분석결과.zip","application/zip")
