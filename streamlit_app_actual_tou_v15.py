...
APP_VERSION = "2026-07-20-actual-tou-v15.0"
...
with T2:
    ...
    if not monitor.empty:
        ...
        st.download_button("고객별 요금 모니터링 CSV",show.to_csv(index=False).encode("utf-8-sig"),f"v15_{year}_{period}_요금모니터링.csv","text/csv")
...
with T3:
    ...
    if period=="연간 종합진단":
        ...
        tdf=pd.DataFrame(rows);tdf["연도"]=tdf["연도"].astype(int).astype(str);tdf=round_table(tdf);st.dataframe(tdf,hide_index=True,use_container_width=True,column_config=dataframe_config(tdf))
...
with T5:
    ...
    ts=D["tariff_summary"].copy();ts["연도"]=ts["연도"].astype(int).astype(str);ts["최저추천비중(%)"]=ts.pop("최저추천비중")*100;ts=round_table(ts);st.dataframe(ts,hide_index=True,use_container_width=True,column_config=dataframe_config(ts))
    fig=px.bar(ts,x="요금제",y="최저추천고객수",color="연도",barmode="group",text="최저추천고객수",category_orders={"연도":["2024","2025"]});st.plotly_chart(fig,use_container_width=True)
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("수요관리우선점수",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v15_100가구_포트폴리오.zip","application/zip")
