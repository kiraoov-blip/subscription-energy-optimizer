...
APP_VERSION = "2026-07-20-actual-tou-v19.0"
...
st.caption(f"앱 버전 {APP_VERSION} · 2024~2025년 고객 712명 · 일반 저압·제주 TOU 공식단가 및 부가요금·세금 반영, 구독료는 모두 포함된 최종가격 · 요금은 원 단위 반올림")
...
with T6:
    ...
    ps=round_table(ptable);st.dataframe(ps.sort_values("수요관리우선점수",ascending=False),hide_index=True,use_container_width=True,column_config=dataframe_config(ps));st.download_button("100가구 분석결과 ZIP",zip_results({"100가구_고객목록.csv":ps.to_csv(index=False).encode("utf-8-sig"),"100가구_변압기제어상세.csv":round_table(control).to_csv(index=False).encode("utf-8-sig")}),"v19_100가구_포트폴리오.zip","application/zip")

with T7:
    st.markdown("""
### v19 핵심 변경점
- **주택용전력 저압**은 120.0·214.6·307.3원/kWh와 계절별 누진구간을 적용합니다.
- **제주 주택용 계시별**은 경·중간·최대부하별 별도 단가와 계약전력 기본요금을 적용합니다.
- 월 사용량은 요금계산단위 1kWh로 반올림하고, 기본·전력량·기후환경·연료비 요금은 원 미만 절사합니다.
- 부가가치세는 원 단위 반올림, 전력산업기반기금과 최종 청구액은 10원 미만 절사합니다.
- 하·동계 슈퍼유저 요금도 요금표 기준으로 반영합니다.
- 고객별 진단·제어의 현재 적용 요금제에 **일반 주택용(저압)·제주 TOU·구독 기본형·구독 프리미엄형**을 모두 포함했습니다.
...
""")
    st.subheader("요금 산식 검증")
    val_basic,val_energy,val_u=residential_base_energy(1000,8)
    val_break=bill_component_breakdown(val_basic,val_energy,val_u)
    val_df=pd.DataFrame([{
        "검증 사례":"8월 주택용 저압 1,000kWh",
        "요금계산 사용량(kWh)":val_u,
        "기본요금(원)":val_break["기본요금(원)"],
        "전력량요금(원)":val_break["전력량요금(원)"],
        "연료비조정액(원)":val_break["연료비조정액(원)"],
        "기후환경요금(원)":val_break["기후환경요금(원)"],
        "부가가치세(원)":val_break["부가가치세(원)"],
        "전력산업기반기금(원)":val_break["전력산업기반기금(원)"],
        "최종 청구액(원)":val_break["최종 청구액(원)"],
    }])
    st.dataframe(round_table(val_df),hide_index=True,use_container_width=True,column_config=dataframe_config(val_df))
    st.success("8월 주택용 저압 1,000kWh의 최종 청구액은 291,320원으로 검증됩니다. 999.7kWh 예측값도 요금계산단위에서 1,000kWh로 반올림되어 동일 금액을 적용합니다.")
    st.caption("제주 TOU는 경부하 22~08시, 중간부하 08~16시, 최대부하 16~22시의 사용량 비중과 별도 계시별 단가를 적용합니다. 1,000kWh 초과 슈퍼유저 구간은 총 초과량에 736.2원/kWh를 적용합니다.")
